...
namespace Server.Mobiles
{
	...
	public class DismissCVendorEntry : ContextMenuEntry
	{
		private ChristmasVendor m_vendor;

		public DismissCVendorEntry( ChristmasVendor vendor ) : base( 6129, 12 ) 
		{
			m_vendor = vendor;
		}
	...
	public class ChristmasVendor : BaseVendor
	{
		...
		[Constructable]
		public ChristmasVendor() : base( "Santa's Senior Elf" )
		{
			Name = "Bernard";
			Frozen = true;
			NameHue = 68;
		...
		{
			AddItem( new ElvenBoots( 1175 ) );
			AddItem( new HalfApron( 1030 ) );
			AddItem( new ElvenPants( 32 ) );
			AddItem( new ElvenDarkShirt() );
		}
		
		public override void OnDoubleClick( Mobile from )
		{
			if (from.HasGump( typeof (GiftRequestGump) ) )
				from.CloseGump( typeof (GiftRequestGump) );
			
			from.SendGump( new GiftRequestGump( 1, this, from, ref m_orders ));
		}
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( ( from == m_owner ) && from.InLOS( this ) )
				list.Add( new DismissCVendorEntry( this ) );
		}
		...
		public ChristmasVendor( Serial serial ) : base( serial )
		{
		}
	...
	public class GiftRequestGump : Gump
	{
		private ChristmasVendor m_vendor;
		...
		public GiftRequestGump( int page, ChristmasVendor vendor, Mobile asker, ref Dictionary<Mobile, string> Order ) : base( 0, 0 )
		{
			m_asker = asker;
			m_vendor = vendor;
			order = Order;
			...
			if ( m_asker != m_vendor.Owner && m_asker.AccessLevel==AccessLevel.Player )
			{
				AddHtml( 264, 80, 200, 24, "Santa's List", false, false );
				AddHtml( 120, 108, 420, 48, "What do you want for Christmas?", false, false );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			{
				from.SendGump( new GiftRequestGump( m_page + 1, m_vendor, m_asker, ref order ) );
			}
			...
			if ( info.ButtonID >= 100 )//Delete record
			{
				...
				if ( order.ContainsKey( mob ) )
				{
					order.Remove( mob );
					from.CloseGump( typeof (GiftRequestGump) );
					from.SendGump( new GiftRequestGump( 1, m_vendor, m_asker, ref order ));
					from.SendMessage( "Order Removed" );
				}// add once order is removed to bring up message gump from orderer
...
{
	public class ChristmasVendorDeed : Item
	{
		[Constructable]
		public ChristmasVendorDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			Name = "Christmas Elf Vendor";
		}
		
		public ChristmasVendorDeed( Serial serial ) : base( serial )
		{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( from.AccessLevel >= AccessLevel.GameMaster )
			{
				...
				ChristmasVendor v = new ChristmasVendor();
