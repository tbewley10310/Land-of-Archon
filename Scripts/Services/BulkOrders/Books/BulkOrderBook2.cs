...
{
	public class BulkOrderBook2 : Item, ISecurable
	{
		...
		[Constructable]
		public BulkOrderBook2() : base( 0x2259 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
            Name = "Bulk Order Book 2";
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( !from.InRange( GetWorldLocation(), 2 ) )
				...
				from.SendGump( new BOBGump2( (PlayerMobile)from, this ) );
		}
			...
			else
			{
				...
				if ( cont != null )
				{
					...
					if ( trade != null && trade.From.Mobile == from )
						trade.To.Mobile.SendGump( new BOBGump2( (PlayerMobile)(trade.To.Mobile), this ) );
					else if ( trade != null && trade.To.Mobile == from )
						trade.From.Mobile.SendGump( new BOBGump2( (PlayerMobile)(trade.From.Mobile), this ) );
				}
			}
		}
		...
		public BulkOrderBook2( Serial serial ) : base( serial )
		{
		}
		...
		private class NameBookEntry : ContextMenuEntry
		{
			...
			public NameBookEntry( Mobile from, BulkOrderBook2 book ) : base( 6216 )
			{
				m_From = from;
				m_Book = book;
			}
		...
		private class NameBookPrompt : Prompt
		{
			...
			public NameBookPrompt( BulkOrderBook2 book )
			{
				m_Book = book;
			}
