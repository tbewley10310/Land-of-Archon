			...
			if( BID == 1 )
			{
				if( GetFlag( Conditions.Category ) )
				{
					if( GetFlag( Conditions.Adding ) )
					{
						...
						{
							c = Int32.Parse( CS );
                            h = Int32.Parse(HS) == 0 ? 1153 : Int32.Parse(HS);
							PGSystem.CategoryList.Add( new PGCategory( NS, Flags, c, h ) );
					...
					else
					{
						...
						else
						{
                            ...
                            PGSystem.CategoryList[m_CurCat].Hue = Int32.Parse(HS) == 0 ? 1153 : Int32.Parse(HS); 
